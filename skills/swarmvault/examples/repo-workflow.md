...
swarmvault memory start "Hand off the auth flow work" --target ./src --agent codex
...
- `wiki/memory/` and `state/memory/tasks/` contain durable task records when `memory start` is used
...
- Use `swarmvault memory resume <id>` when a future agent needs the task summary, decisions, evidence, and follow-ups.
