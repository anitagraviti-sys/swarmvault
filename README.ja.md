...
各 compile は、そのまま投稿しやすい share card も生成します：
...
- **Share card** —— `wiki/graph/share-card.md` と `swarmvault graph share --post` によるコピー可能な初回サマリー
...
ローカル repo や docs ツリーを最短で一度見たい場合は、`swarmvault scan ./path --no-serve` を使います。現在のディレクトリを vault として初期化し、そのディレクトリを取り込み、compile まで実行し、`--no-serve` ならグラフビューアは起動しません。`wiki/graph/share-card.md` も残るため、`swarmvault graph share --post` で短い共有用サマリーを出力できます。
...
**投稿しやすい share card** - すべての compile が `wiki/graph/share-card.md` を生成し、`swarmvault graph share --post` はソース/ページ/ノード数、top hubs、最も意外なリンクを含む短いサマリーを出力します。
