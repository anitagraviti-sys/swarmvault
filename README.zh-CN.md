...
每次 compile 还会写出一个适合直接发布的 share card：
...
- **Share card** —— `wiki/graph/share-card.md` 和 `swarmvault graph share --post`，用于复制首轮扫描摘要
...
想要对本地仓库或文档树做最快的一次性扫描？`swarmvault scan ./path --no-serve` 会将当前目录初始化为 vault，导入该目录并完成编译；加上 `--no-serve` 时不会启动图谱查看器。它还会留下 `wiki/graph/share-card.md`，之后运行 `swarmvault graph share --post` 就能复制一段紧凑摘要。
...
**可直接发布的 share card** - 每次 compile 都会写入 `wiki/graph/share-card.md`，`swarmvault graph share --post` 会打印包含来源/页面/节点数量、top hubs 和最意外连接的简短摘要，方便分享。
