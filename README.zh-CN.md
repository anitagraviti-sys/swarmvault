...
- **Vault doctor 和工作台** —— `swarmvault doctor [--repair]`、MCP `doctor_vault` 与图谱查看器健康栏可从一个入口检查 graph、retrieval、review、watch、migration、managed sources 和 task 状态
...
需要在交给 agent 或打开 viewer 前做一次快速健康检查？`swarmvault doctor` 会检查 graph、retrieval、review queue、watch status、migration state、managed sources 和 task ledger。加上 `--repair` 可重建安全的派生 retrieval artifact。`swarmvault graph serve` 也会在工作台里显示同一份 doctor 摘要，并提供 capture、context-pack 和 task-start 操作。
...
**Vault doctor 与工作台** - `swarmvault doctor [--repair]` 会检查 graph artifact、retrieval、review queue、watch state、migration、managed sources、source/page count 和 task 状态。图谱查看器工作台会显示同一份健康摘要，并提供 repair、丰富 capture、context-pack 和 task-start 操作。
