...
3. Query the compiled vault and optionally save answers back into the wiki
4. Keep the vault healthy with linting, inbox automation, and MCP access
...
- `swarmvault init`: create a workspace and default config
...
- `swarmvault compile`: build wiki pages, graph data, and the search index
- `swarmvault query "<question>" --save`: answer questions and optionally persist them into `wiki/outputs/`
...
Most "chat with your docs" tools answer a question and then throw away the work. SwarmVault treats the vault itself as the product. The markdown pages, saved outputs, graph edges, manifests, and freshness state are durable artifacts you can inspect, diff, and keep improving.
